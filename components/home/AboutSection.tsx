...
// Restaurant images for the sphere - Using ALL local images for best performance and variety
const localImages: ImageData[] = [
  ...
  },
  {
    ...
    title: 'Master Chef',
    description: 'Expert chefs creating culinary masterpieces'
  },
  {
    id: 'img-10',
...
// Use images directly - if you have 30+ images, no need to duplicate
// If you have fewer images, they will be duplicated automatically for better sphere coverage
const restaurantImages: ImageData[] = 
  localImages.length >= 20 
    ? localImages // Use directly if 20+ images (no duplication needed)
    : [
        // Duplicate if fewer images for fuller sphere effect
        ...localImages,
        ...localImages.map((img, idx) => ({ ...img, id: `${img.id}-dup-${idx}` })),
        ...localImages.map((img, idx) => ({ ...img, id: `${img.id}-dup2-${idx}` })),
        ...localImages.map((img, idx) => ({ ...img, id: `${img.id}-dup3-${idx}` })),
        ...localImages.map((img, idx) => ({ ...img, id: `${img.id}-dup4-${idx}` })),
      ]
...
export default function AboutSection() {
  ...
  return (
    <section
      ref={containerRef}
      className="relative z-10 pt-20 pb-24 -mt-40 overflow-visible bg-gradient-to-b from-dark-900 via-dark-900 to-dark-800"
      style={{ position: 'relative' }}
    >
      <div className="container-custom">
        <div className="grid lg:grid-cols-2 gap-8 items-center">
          {/* 3D Image Sphere - Larger and more prominent */}
          ...
          >
            ...
            >
              ...
              local and international suppliers. Explore our interactive gallery above to see our culinary journey.
            </motion.p>
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
