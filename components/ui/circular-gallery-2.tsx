...
class App {
  ...
  onMouseMove(e: MouseEvent) {
    if (this.isDown) return; // Don't interfere with drag
    const rect = this.container.getBoundingClientRect();
    const centerX = rect.left + rect.width / 2;
    const mouseX = e.clientX;
    const offsetX = (mouseX - centerX) / rect.width; // Normalized -1 to 1
    this.mouseX = offsetX;
    
    // Update scroll target based on mouse position
    if (this.medias && this.medias[0]) {
      const maxScroll = this.medias[0].widthTotal / 2;
      this.scroll.target = -offsetX * maxScroll * 0.5; // 0.5 is sensitivity multiplier
    }
  }
  ...
  onMouseLeave() {
    ...
    // Optionally snap to nearest item when mouse leaves
    this.onCheck();
  }
  ...
  update() {
    // Pause updates when page is hidden or not in view
    if (document.hidden) {
